    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::Note(note) => {
                    validate_create_note(EntryCreationAction::Create(action), note)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::Note(note) => {
                    validate_create_note(EntryCreationAction::Update(action), note)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::Note(note) => {
                        ...
                        let original_note = match Note::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Note from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_note(action, note, original_create_action, original_note)
                    }
                }
            }
            _ => Ok(ValidateCallbackResult::Valid),
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::Note(original_note) => validate_delete_note(
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::ListNotes => {
                validate_create_link_list_notes(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::ListNotes => validate_delete_link_list_notes(
                action,
                original_action,
                base_address,
                target_address,
                    ...
                    match app_entry {
                        EntryTypes::Note(note) => {
                            ...
                            if let ValidateCallbackResult::Valid = result {
                                ...
                                validate_update_note(action, note, original_action, original_note)
                            } else {
                                Ok(result)
                            }
                        }
